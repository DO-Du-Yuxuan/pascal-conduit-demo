...
export function devicePlanAnchor(nodes: Record<string, NodeData>, device: NetworkDevice): Point {
  ...
  if (wall?.type !== 'wall' || !Array.isArray(wall.start) || !Array.isArray(wall.end)) return point2(device.position.position);
  const start: Point = [Number(wall.start[0]), Number(wall.start[1])], end: Point = [Number(wall.end[0]), Number(wall.end[1])];
  const dx = end[0] - start[0], dz = end[1] - start[1], length = Math.hypot(dx, dz);
  if (!Number.isFinite(length) || length < 1e-6) return point2(device.position.position);
  const tangent: Point = [dx / length, dz / length], raw = point2(device.position.position), along = typeof attachment.localPosition?.[0] === 'number' ? attachment.localPosition[0] : (raw[0] - start[0]) * tangent[0] + (raw[1] - start[1]) * tangent[1];
...
export function buildPointPositionDimensionReport(nodes:Record<string,NodeData>,overlay:ConduitOverlayDocument,levelId:string,context=createPlanContext(nodes,overlay)):{dimensions:PointPositionDimension[];notices:PlanNotice[]}{
  ...
  const boundary=(entry:PlaneEntry,rowCross:number,side:-1|1)=>{const origin:Point=[entry.axis[0]*entry.scalar+entry.normal[0]*rowCross,entry.axis[1]*entry.scalar+entry.normal[1]*rowCross],rayEnd:Point=[origin[0]+entry.axis[0]*side*1e5,origin[1]+entry.axis[1]*side*1e5];return straightWalls.flatMap(wall=>{const start:Point=[Number(wall.start![0]),Number(wall.start![1])],end:Point=[Number(wall.end![0]),Number(wall.end![1])],hit=segmentIntersection(origin,rayEnd,start,end);if(!hit||hit.t<=1e-9||hit.u<0||hit.u>1)return [];const length=Math.hypot(end[0]-start[0],end[1]-start[1]);if(length<1e-6)return [];const along=hit.u*length,opening=(openingsByWall.get(wall.id)??[]).find(item=>along>=item.start&&along<=item.end),wallDirection:Point=[(end[0]-start[0])/length,(end[1]-start[1])/length],half=Math.max(0,Number(wall.thickness)||DEFAULT_WALL_THICKNESS_METERS)/2,wallNormal:Point=[-wallDirection[1]*half,wallDirection[0]*half],corners:Point[]=[[start[0]+wallNormal[0],start[1]+wallNormal[1]],[end[0]+wallNormal[0],end[1]+wallNormal[1]],[end[0]-wallNormal[0],end[1]-wallNormal[1]],[start[0]-wallNormal[0],start[1]-wallNormal[1]]],faceHit=corners.flatMap((corner,index)=>{const candidate=segmentIntersection(origin,rayEnd,corner,corners[(index+1)%corners.length]!);return candidate&&candidate.t>1e-9&&candidate.u>=-1e-8&&candidate.u<=1+1e-8?[candidate]:[];}).sort((a,b)=>a.t-b.t)[0];if(!faceHit)return [];const boundaryScalar=dot([start[0]+(end[0]-start[0])*hit.u,start[1]+(end[1]-start[1])*hit.u],entry.axis)-side*half,openingPoint:Point=[entry.axis[0]*boundaryScalar+entry.normal[0]*rowCross,entry.axis[1]*boundaryScalar+entry.normal[1]*rowCross],facePoint:Point=[origin[0]+(rayEnd[0]-origin[0])*faceHit.t,origin[1]+(rayEnd[1]-origin[1])*faceHit.t],edge=opening?(Math.abs(along-opening.start)<=Math.abs(opening.end-along)?opening.start:opening.end):along,witness:Point=[start[0]+wallDirection[0]*edge-entry.axis[0]*side*half,start[1]+wallDirection[1]*edge-entry.axis[1]*side*half],point=opening?openingPoint:facePoint;return [{wall,point,witness:opening?witness:facePoint,kind:opening?'opening-edge' as const:'wall-face' as const,openingId:opening?.id,distance:pointDistance(origin,point),direction:entry.axis,normal:entry.normal}];}).sort((a,b)=>a.distance-b.distance||a.wall.id.localeCompare(b.wall.id))[0]??null;};
  for(const group of planeGroups.values()){
    ...
    for(const row of rows){const sorted=[...row].sort((a,b)=>a.scalar-b.scalar||a.device.id.localeCompare(b.device.id)),runs:PlaneEntry[][]=[];for(const entry of sorted){const current=runs[runs.length-1];if(!current||crossesSolidWall(current[current.length-1]!.center,entry.center))runs.push([entry]);else current.push(entry);}
      for(const run of runs){const first=run[0]!,last=run[run.length-1]!,rowCross=run.reduce((sum,item)=>sum+item.cross,0)/run.length,left=boundary(first,rowCross,-1),right=boundary(last,rowCross,1),normal=first.normal,lane=0,axisId=`${Math.round(first.axis[0]*1000)}:${Math.round(first.axis[1]*1000)}`;
        ...
        if(left){const center=projected(first);dimensions.push({id:`${first.device.id}:position:axis:${axisId}:from:${left.kind}:${left.wall.id}:${left.openingId??'face'}`,sourceId:first.device.id,levelId,wallId:left.wall.id,reference:left.point,center,referenceWitness:left.witness,centerWitness:first.center,direction:left.direction,normal:left.normal,lane,valueMeters:pointDistance(left.point,center),referenceKind:left.kind,relatedIds:[first.device.id,left.wall.id,...(left.openingId?[left.openingId]:[])],measurementBasis:'derived',confidence:'limited',assumptions:['同专业、同类型、同安装面的近似共线点位形成闭合定位尺寸链；边界为该方向首先到达的可靠墙面或洞口边。']});}
        run.slice(1).forEach((entry,index)=>{const previous=run[index]!,reference=projected(previous),center=projected(entry);dimensions.push({id:`${entry.device.id}:position:axis:${axisId}:from:device:${previous.device.id}`,sourceId:entry.device.id,levelId,reference,center,referenceWitness:previous.center,centerWitness:entry.center,direction:first.axis,normal,lane,valueMeters:pointDistance(reference,center),referenceKind:'device-center',relatedIds:[previous.device.id,entry.device.id],measurementBasis:'derived',confidence:'limited',assumptions:['点位中心垂直于尺寸链方向的模型偏差不超过 100 mm，按同一排或同一列表达。']});});
        if(right){const center=projected(last);dimensions.push({id:`${last.device.id}:position:axis:${axisId}:to:${right.kind}:${right.wall.id}:${right.openingId??'face'}`,sourceId:last.device.id,levelId,wallId:right.wall.id,reference:right.point,center,referenceWitness:right.witness,centerWitness:last.center,direction:right.direction,normal:right.normal,lane,valueMeters:pointDistance(right.point,center),referenceKind:right.kind,relatedIds:[last.device.id,right.wall.id,...(right.openingId?[right.openingId]:[])],measurementBasis:'derived',confidence:'limited',assumptions:['同专业、同类型、同安装面的近似共线点位形成闭合定位尺寸链；边界为该方向首先到达的可靠墙面或洞口边。']});}
