...
export function buildPointPositionDimensionReport(nodes:Record<string,NodeData>,overlay:ConduitOverlayDocument,levelId:string,context=createPlanContext(nodes,overlay)):{dimensions:PointPositionDimension[];notices:PlanNotice[]}{
  ...
  for(const device of overlay.devices){
    ...
    const center=point2(device.position.position),candidates=straightWalls.flatMap(wall=>{const start:Point=[Number(wall.start![0]),Number(wall.start![1])],end:Point=[Number(wall.end![0]),Number(wall.end![1])],edge:Point=[end[0]-start[0],end[1]-start[1]],length=Math.hypot(...edge);if(length<1e-6)return [];const along:Point=[edge[0]/length,edge[1]/length],raw=(center[0]-start[0])*along[0]+(center[1]-start[1])*along[1];if(raw<0||raw>length)return [];const linePoint:Point=[start[0]+along[0]*raw,start[1]+along[1]*raw],delta:Point=[center[0]-linePoint[0],center[1]-linePoint[1]],distance=Math.hypot(...delta);if(distance<1e-6)return [];return [{direction:[delta[0]/distance,delta[1]/distance] as Point,value:Math.max(0,distance-Math.max(0,Number(wall.thickness)||0)/2)}];}).sort((a,b)=>a.value-b.value);
    const axes:typeof candidates=[];for(const candidate of candidates){if(axes.every(axis=>Math.abs(dot(candidate.direction,axis.direction))<.2))axes.push(candidate);if(axes.length===2)break;}
    if(axes.length<2)notices.push({sourceId:device.id,levelId,text:'该方向定位尺寸链未闭合',anchor:center});
    axes.forEach(candidate=>{const axis=canonical(candidate.direction),normal:Point=[-axis[1],axis[0]];entries.push({device,center,axis,normal,scalar:dot(center,axis),cross:dot(center,normal)});});
  }
  ...
  const boundary=(entry:PlaneEntry,rowCross:number,side:-1|1)=>{const origin:Point=[entry.axis[0]*entry.scalar+entry.normal[0]*rowCross,entry.axis[1]*entry.scalar+entry.normal[1]*rowCross],rayEnd:Point=[origin[0]+entry.axis[0]*side*1e5,origin[1]+entry.axis[1]*side*1e5];return straightWalls.flatMap(wall=>{const start:Point=[Number(wall.start![0]),Number(wall.start![1])],end:Point=[Number(wall.end![0]),Number(wall.end![1])],hit=segmentIntersection(origin,rayEnd,start,end);if(!hit||hit.t<=1e-9||hit.u<0||hit.u>1)return [];const length=Math.hypot(end[0]-start[0],end[1]-start[1]),along=hit.u*length,opening=(openingsByWall.get(wall.id)??[]).find(item=>along>=item.start&&along<=item.end),wallDirection:Point=[(end[0]-start[0])/length,(end[1]-start[1])/length],half=Math.max(0,Number(wall.thickness)||0)/2,boundaryScalar=dot([start[0]+(end[0]-start[0])*hit.u,start[1]+(end[1]-start[1])*hit.u],entry.axis)-side*half,projectedPoint:Point=[entry.axis[0]*boundaryScalar+entry.normal[0]*rowCross,entry.axis[1]*boundaryScalar+entry.normal[1]*rowCross],edge=opening?(Math.abs(along-opening.start)<=Math.abs(opening.end-along)?opening.start:opening.end):along,witness:Point=[start[0]+wallDirection[0]*edge-entry.axis[0]*side*half,start[1]+wallDirection[1]*edge-entry.axis[1]*side*half],point=opening?witness:projectedPoint,delta:Point=[origin[0]-point[0],origin[1]-point[1]],distance=Math.hypot(...delta),direction:Point=distance>1e-6?[delta[0]/distance,delta[1]/distance]:entry.axis,normal:Point=[-direction[1],direction[0]];return [{wall,point,witness,kind:opening?'opening-edge' as const:'wall-face' as const,openingId:opening?.id,distance,direction,normal}];}).sort((a,b)=>a.distance-b.distance||a.wall.id.localeCompare(b.wall.id))[0]??null;};
  let planeLane=0;
