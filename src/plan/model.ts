...
export function buildPointPositionDimensionReport(nodes:Record<string,NodeData>,overlay:ConduitOverlayDocument,levelId:string,context=createPlanContext(nodes,overlay)):{dimensions:PointPositionDimension[];notices:PlanNotice[]}{
  ...
  [...wallGroups.values()].sort((a,b)=>a[0]!.wall.id.localeCompare(b[0]!.wall.id)||a[0]!.device.deviceType.localeCompare(b[0]!.device.deviceType)).forEach((group,lane)=>{
    ...
    for(const run of runs){const runFirst=run[0]!,runLast=run[run.length-1]!,left=candidates.filter(candidate=>candidate.scalar<=runFirst.scalar+1e-6).slice(-1)[0],right=candidates.find(candidate=>candidate.scalar>=runLast.scalar-1e-6);if(!left||!right)continue;
      const facePoint=(scalar:number):Point=>{const half=Math.max(0,Number(runFirst.wall.thickness)||DEFAULT_WALL_THICKNESS_METERS)/2;return [runFirst.start[0]+runFirst.direction[0]*scalar+runFirst.normal[0]*half,runFirst.start[1]+runFirst.direction[1]*scalar+runFirst.normal[1]*half];};
      let previousPoint:Point=facePoint(left.scalar),previousWitness=previousPoint,previousId:string|undefined;
      ...
      const boundaryPoint:Point=facePoint(right.scalar),lastPoint:Point=runLast.center;dimensions.push({id:`${runLast.device.id}:position:wall:${runLast.wall.id}:to:${right.kind}:${right.sourceId}`,sourceId:runLast.device.id,levelId,wallId:runLast.wall.id,reference:boundaryPoint,center:lastPoint,referenceWitness:boundaryPoint,centerWitness:runLast.center,direction:runLast.direction,normal:runLast.normal,lane,valueMeters:pointDistance(boundaryPoint,lastPoint),referenceKind:right.kind,relatedIds:[runLast.device.id,runLast.wall.id,...(right.sourceId!==runLast.wall.id?[right.sourceId]:[])],measurementBasis:'derived',confidence:'high',assumptions:['同墙同类型点位形成闭合定位尺寸链；门窗洞口将尺寸链分开，端部止于洞口边或墙端。']});
    }
  });
