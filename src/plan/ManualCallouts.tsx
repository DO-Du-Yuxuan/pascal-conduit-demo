  ...
  return <g className="manual-callouts">
    ...
    {callouts.map(item=>{const selected=selectedId===item.id,fontSize=.2*annotationScale,lines=item.text.split('\n'),width=Math.max(.9,...lines.map(value=>Array.from(value).reduce((sum,char)=>sum+(char.charCodeAt(0)>255?fontSize:fontSize*.58),0)+.28*annotationScale)),lineHeight=.3*annotationScale,height=Math.max(.34*annotationScale,lines.length*lineHeight);return <g key={item.id} data-manual-callout={item.id} onClick={event=>{event.stopPropagation();onSelect(item.id);}}>
      ...
      <g transform={`translate(${item.label[0]} ${item.label[1]}) rotate(${-rotation})`} onPointerDown={event=>{if((event.target as Element).closest('textarea'))return;onMoveStart(item.id,event);}} onPointerMove={onMove} onPointerUp={onMoveEnd} onPointerCancel={onMoveEnd} style={{cursor:'move'}}>
        ...
        {editing?.id===item.id?<foreignObject x={-width/2+.05} y={-height/2+.02} width={width-.1} height={height-.04}><textarea autoFocus placeholder="输入标注文字" value={editing.value} onChange={event=>setEditing({...editing,value:event.target.value})} onBlur={()=>{if(cancelledEdit.current){cancelledEdit.current=false;return;}save();}} onKeyDown={event=>{event.stopPropagation();if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();save();}if(event.key==='Escape'){event.preventDefault();cancelledEdit.current=true;if(autoEditId===item.id)onDelete(item.id);setEditing(null);onEditFinished();}}} style={{width:'100%',height:'100%',border:0,padding:0,resize:'none',fontSize:`${14*annotationScale}px`,lineHeight:1.35,background:'#fff'}}/></foreignObject>:<text textAnchor="middle" fontSize={fontSize} fill="#343434" onDoubleClick={event=>{event.stopPropagation();setEditing({id:item.id,value:item.text});}}>{lines.map((line,index)=><tspan key={index} x="0" y={-height/2+lineHeight*(index+.72)}>{line}</tspan>)}</text>}
      </g>
    </g>;})}
  </g>;
}
