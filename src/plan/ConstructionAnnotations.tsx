...
export function useConstructionPlan({ nodes, overlay, levelId, hiddenNodeIds, unit, rotation, viewBox, planRef, selectedId, exterior, dimensionsVisible, measurements, systemVisibility, sensorVisible, devicesVisible, hvacVisible = true, annotationScale }: {
  ...
  systemVisibility?: Readonly<Record<RoutingSystem, boolean>>; sensorVisible?: boolean; devicesVisible?: boolean; hvacVisible?: boolean; annotationScale: number;
}) {
  ...
  const annotationReport=useMemo(()=>{
    ...
    const report=buildPlanAnnotations(nodes,overlay,levelId,unit,context,hvacVisible);
    return {...report,annotations:report.annotations.filter(annotation=>!overlay.hiddenConstructionAnnotationIds.includes(annotation.id)).map(annotation=>preserveAnnotationPresentation(annotation,overlay.constructionAnnotationLabelPlacementSignatures[annotation.id]))};
  },[nodes,overlay,levelId,unit,context,devicesVisible,hvacVisible]);
  ...
  const hvacDimensions=useMemo(()=>overlay&&hvacVisible?buildHvacPositionDimensions(nodes,overlay,levelId):[],[nodes,overlay,levelId,hvacVisible]);
  const positionDimensions=[...positionReport.dimensions,...hvacDimensions].filter(dimension=>!overlay?.hiddenPointPositionDimensionIds.includes(dimension.id));
...
export function ConstructionAnnotations({plan,rotation,onSelect,onLabelPositionChange,toPlanPoint,selectionClearVersion=0}: {plan:ConstructionPlan;rotation:number;onSelect:(id:string|null)=>void;onLabelPositionChange?:(id:string,label:Point,signature:string)=>void;toPlanPoint?:(clientX:number,clientY:number)=>Point|null;selectionClearVersion?:number}) {
  const overlay=useOverlayStore(state=>state.overlay),commit=useOverlayStore(state=>state.commit),undo=useOverlayStore(state=>state.undo),redo=useOverlayStore(state=>state.redo),[editing,setEditing]=useState<{ids:string[];value:string;left:number;top:number;width:number}|null>(null),[selectedAnnotationId,setSelectedAnnotationId]=useState<string|null>(null),[draftLabels,setDraftLabels]=useState<Record<string,Point>>({}),dragRef=useRef<{id:string;pointerId:number}|null>(null),lineHeight=.3*plan.annotationScale,fontSize=.2*plan.annotationScale;
  useEffect(()=>{setSelectedAnnotationId(null);},[selectionClearVersion]);
  useEffect(()=>{const onKeyDown=(event:KeyboardEvent)=>{if(!selectedAnnotationId||editing||event.isComposing||event.target instanceof HTMLInputElement||event.target instanceof HTMLTextAreaElement)return;const meta=event.metaKey||event.ctrlKey,key=event.key.toLowerCase();if(event.key==='Escape'){setSelectedAnnotationId(null);onSelect(null);}else if((event.key==='Delete'||event.key==='Backspace')&&!meta&&overlay){event.preventDefault();event.stopImmediatePropagation();commit({...overlay,hiddenConstructionAnnotationIds:[...new Set([...overlay.hiddenConstructionAnnotationIds,selectedAnnotationId])]});setSelectedAnnotationId(null);}else if(meta&&key==='z'){event.preventDefault();event.stopImmediatePropagation();event.shiftKey?redo():undo();}else if(meta&&key==='y'){event.preventDefault();event.stopImmediatePropagation();redo();}};window.addEventListener('keydown',onKeyDown,true);return()=>window.removeEventListener('keydown',onKeyDown,true);},[selectedAnnotationId,editing,overlay,commit,undo,redo,onSelect]);
  const rename=(ids:string[],value:string)=>{const selected=overlay?.devices.filter(device=>ids.includes(device.id))??[],thermostats=overlay?.hvac.thermostats.filter(item=>ids.includes(item.id))??[],trimmed=value.trim(),name=trimmed||(thermostats.length?'空调温控器':selected.every(device=>device.deviceType==='sensor')?'传感器':'');if(overlay&&name)commit({...overlay,devices:overlay.devices.map(device=>ids.includes(device.id)?{...device,name}:device),hvac:{...overlay.hvac,thermostats:overlay.hvac.thermostats.map(item=>ids.includes(item.id)?{...item,name}:item)}});setEditing(null);};
  ...
  return <><g className="construction-annotations" aria-label="施工标注">
    {plan.layout.placed.map(({annotation:a,innerBend,boundary,label:placedLabel,textWidth,textHeight,outwardNormal,manual})=>{
      ...
      return <g key={a.id} data-annotation={a.kind} data-construction-annotation-selected={selected||undefined} data-source-id={a.sourceId} data-rule-side={ruleSide} onClick={e=>{e.stopPropagation();setSelectedAnnotationId(a.id);onSelect(a.sourceId);}}>
        ...
        <g data-construction-annotation-panel={a.id} transform={`translate(${label[0]} ${label[1]}) rotate(${-rotation})`} onPointerDown={event=>{if(!toPlanPoint||!onLabelPositionChange||(event.target as Element).closest('text, input, foreignObject'))return;event.stopPropagation();event.currentTarget.setPointerCapture(event.pointerId);dragRef.current={id:a.id,pointerId:event.pointerId};}} onPointerMove={event=>{const drag=dragRef.current;if(!drag||drag.pointerId!==event.pointerId)return;const point=toPlanPoint?.(event.clientX,event.clientY);if(point)setDraftLabels(labels=>({...labels,[drag.id]:point}));}} onPointerUp={event=>{const drag=dragRef.current;if(!drag||drag.pointerId!==event.pointerId)return;const point=toPlanPoint?.(event.clientX,event.clientY)??draftLabels[drag.id]??label;if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);dragRef.current=null;setDraftLabels(labels=>{const next={...labels};delete next[drag.id];return next;});onLabelPositionChange?.(drag.id,point,annotationPlacementSignature(a));}} onPointerCancel={event=>{const drag=dragRef.current;if(!drag||drag.pointerId!==event.pointerId)return;if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);dragRef.current=null;setDraftLabels(labels=>{const next={...labels};delete next[drag.id];return next;});}}>
          ...
          <rect x={-textWidth/2-.04} y={-textHeight/2-.08} width={textWidth+.08} height={textHeight+.16} fill="#fff" fillOpacity=".96" stroke={selected?'#e75c3c':'none'} strokeWidth={selected?'.03':'0'}/>
          <line data-callout-rule x1={edgeX} y1={-textHeight/2} x2={edgeX} y2={textHeight/2} stroke={selected?'#e75c3c':'#343434'} strokeWidth={selected?'1.5':'1'} vectorEffect="non-scaling-stroke"/>
