...
export function ConstructionAnnotations({plan,rotation,onSelect,onLabelPositionChange,toPlanPoint}: {plan:ConstructionPlan;rotation:number;onSelect:(id:string|null)=>void;onLabelPositionChange?:(id:string,label:Point,signature:string)=>void;toPlanPoint?:(clientX:number,clientY:number)=>Point|null}) {
  ...
  useEffect(()=>{const onKeyDown=(event:KeyboardEvent)=>{if(!selectedAnnotationId||editing||event.isComposing||event.target instanceof HTMLInputElement||event.target instanceof HTMLTextAreaElement)return;const meta=event.metaKey||event.ctrlKey,key=event.key.toLowerCase();if((event.key==='Delete'||event.key==='Backspace')&&!meta&&overlay){event.preventDefault();event.stopImmediatePropagation();commit({...overlay,hiddenConstructionAnnotationIds:[...new Set([...overlay.hiddenConstructionAnnotationIds,selectedAnnotationId])]});setSelectedAnnotationId(null);}else if(meta&&key==='z'){event.preventDefault();event.stopImmediatePropagation();event.shiftKey?redo():undo();}else if(meta&&key==='y'){event.preventDefault();event.stopImmediatePropagation();redo();}};window.addEventListener('keydown',onKeyDown,true);return()=>window.removeEventListener('keydown',onKeyDown,true);},[selectedAnnotationId,editing,overlay,commit,undo,redo]);
