...
const ConduitPlanPermanent = React.memo(function ConduitPlanPermanent({ overlay, levelId, selectedId, onSelect, context, scale, rotation, devicesVisible, annotationScale, deviceVariants }: { overlay: ConduitOverlayDocument; levelId: string; selectedId: string | null; onSelect: (id: string | null) => void; context: PlanContext; scale: number; rotation: number; devicesVisible: boolean; annotationScale: number; deviceVariants: Readonly<Record<string, string>> }) {
  ...
  return <g className="conduit-plan-permanent" fill="none" strokeWidth={1.8 / scale} strokeLinecap="round" strokeLinejoin="round">
    ...
    {devicesVisible && overlay.devices.filter(d => context.deviceVisible(d) && context.deviceLevel(d) === levelId).map(d => { const symbolRotation=devicePlanRotation(d,rotation),variant=deviceVariants[d.id]; return <g key={d.id} data-device-symbol={d.deviceType} transform={`translate(${d.position.position[0]} ${d.position.position[2]}) rotate(${symbolRotation}) scale(${.018*annotationScale})`} stroke={color(d.id,d.systems.find(s=>context.systemVisibility[s]) ?? d.systems[0])} strokeWidth="1.5" fill="#fff" onClick={select(d.id)}><rect x="-11" y="-11" width="22" height="22" fill="transparent" stroke="none"/><DeviceSymbol type={d.deviceType}/>{variant&&<g transform={`rotate(${-symbolRotation-rotation} 14 -10)`}><rect x="8" y="-16" width="12" height="12" rx="2" stroke="none" fill="#fff" fillOpacity=".92"/><text data-device-variant={variant} x="14" y="-10" textAnchor="middle" dominantBaseline="middle" fontSize="10" fontWeight="700" stroke="none" fill="#343434">{variant}</text></g>}</g>;})}
...
export function ConduitPlanOverlay({ overlay, levelId, selectedId, onSelect, context, scale, rotation, devicesVisible = true, annotationScale = 1, deviceVariants = {} }: { overlay: ConduitOverlayDocument | null; levelId: string; selectedId: string | null; onSelect: (id: string | null) => void; context: PlanContext | null; scale: number; rotation: number; devicesVisible?: boolean; annotationScale?: number; deviceVariants?: Readonly<Record<string, string>> }) {
  ...
  return <g className="conduit-plan-overlay" aria-label="只读管线平面图">
    <ConduitPlanPermanent overlay={overlay} levelId={levelId} selectedId={selectedId} onSelect={onSelect} context={context} scale={scale} rotation={rotation} devicesVisible={devicesVisible} annotationScale={annotationScale} deviceVariants={deviceVariants} />
