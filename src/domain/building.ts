...
export type BuildingScene = { raw: { nodes?: Record<string, PascalNode>; rootNodeIds?: string[]; installedPlugins?: string[] }; nodes: Record<string, PascalNode>; rootNodeIds: string[]; levelIds: string[]; diagnostics: string[] };
...
export function parseBuilding(raw: unknown): BuildingScene {
  ...
  const rootNodeIds = declaredRoots.length ? declaredRoots : derivedRoots, diagnostics: string[] = [];
