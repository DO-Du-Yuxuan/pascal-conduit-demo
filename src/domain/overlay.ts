...
export function createEmptyOverlay(fileName: string, sha256: string): ConduitOverlayDocument {
  return { schemaVersion: "2.2", source: { fileName, sha256 }, settings: { colors: Object.fromEntries(SYSTEMS.map((system) => [system, SYSTEM_DEFAULTS[system].color])) as Record<RoutingSystem, string>, visibleSystems: Object.fromEntries(SYSTEMS.map((system) => [system, true])) as Record<RoutingSystem, boolean>, sensorVisible: true, bendRadiusMm: 200, stockLengthMm: 4000, junctionBoxSizeMm: [86, 86, 50] }, segments: [], fittings: [], junctionBoxes: [], devices: [], circuits: [], lightingControlGroups: [], manualCallouts: [], pointPositionDimensionLabelPositions: {}, constructionAnnotationLabelPositions: {}, constructionAnnotationLabelPlacementSignatures: {}, surfaceChases: [], penetrations: [], installationReferencePlanes: [] };
}
...
export function parseOverlay(raw: unknown): ConduitOverlayDocument {
  ...
  return { schemaVersion: "2.2", source: parsed.source, segments, fittings, junctionBoxes, devices, circuits, lightingControlGroups, manualCallouts: parsed.manualCallouts ?? [], pointPositionDimensionLabelPositions: parsed.pointPositionDimensionLabelPositions ?? {}, constructionAnnotationLabelPositions: parsed.constructionAnnotationLabelPositions ?? {}, constructionAnnotationLabelPlacementSignatures: parsed.constructionAnnotationLabelPlacementSignatures ?? {}, surfaceChases: [...migratedChases, ...(parsed.surfaceChases ?? [])], penetrations, installationReferencePlanes: parsed.installationReferencePlanes ?? [], settings: { colors, visibleSystems, sensorVisible: parsed.settings.sensorVisible ?? true, bendRadiusMm: parsed.settings.bendRadiusMm ?? 200, stockLengthMm: parsed.settings.stockLengthMm ?? 4000, junctionBoxSizeMm: parsed.settings.junctionBoxSizeMm ?? [86, 86, 50] } };
}
