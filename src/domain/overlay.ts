...
export type HostAttachment = { hostId: string; hostKind: HostKind; surface: string; normal: Vec3; levelId: string | null };
...
export type ConduitOverlayDocument = {
  ...
  settings: { colors: Record<RoutingSystem, string> };
...
const attachmentSchema = z.object({ hostId: z.string(), hostKind: z.enum(["wall", "slab", "ceiling"]), surface: z.string(), normal: vec3Schema, levelId: z.string().nullable() });
...
const overlaySchema = z.object({ schemaVersion: z.literal("1.0"), source: z.object({ fileName: z.string(), sha256: z.string() }), settings: z.object({ colors: z.record(systemSchema, z.string()) }), segments: z.array(segmentSchema), fittings: z.array(fittingSchema), wallChases: z.array(wallChaseSchema), penetrations: z.array(penetrationSchema) });

export function createEmptyOverlay(fileName: string, sha256: string): ConduitOverlayDocument {
  return { schemaVersion: "1.0", source: { fileName, sha256 }, settings: { colors: Object.fromEntries(SYSTEMS.map((system) => [system, SYSTEM_DEFAULTS[system].color])) as Record<RoutingSystem, string> }, segments: [], fittings: [], wallChases: [], penetrations: [] };
}
