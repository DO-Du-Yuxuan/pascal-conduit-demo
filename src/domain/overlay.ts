...
export type HostKind = "wall" | "slab" | "ceiling" | "beam";
...
export type NetworkDevice = { id: string; type: "network-device"; deviceType: NetworkDeviceType; name: string; position: RoutePoint; sizeMm: [number, number, number]; orientation: Vec3; frame?: DeviceFrame; mount?: DeviceMount; positioning?: DevicePositionReference; sprinklerDirection?: SprinklerDirection; systems: RoutingSystem[]; ports: NetworkPort[]; createdAt: string };
export type InstallationReferencePlane = { levelId: string; elevationMm: number; basis: "finished-floor"; derived?: boolean };
export type LayoutReferencePlane = { levelId: string; visible: boolean; elevationMm: number; basis: "largest-area-ceiling" | "derived-default-2700mm" | "explicit"; sourceCeilingId?: string };
...
export type HvacIndoorUnit = { id: string; type: "indoor-air-handling-unit"; name: string; position: RoutePoint; mount?: DeviceMount; sizeMm: [number, number, number]; sectionMm: [number, number]; rotationYDegrees: number; createdAt: string };
export type HvacDuctSegment = { id: string; start: RoutePoint; end: RoutePoint };
export type HvacDuct = { id: string; type: "hvac-duct"; indoorUnitId: string; system: HvacSystem; segmentIds: string[]; createdAt: string };
export type HvacOutletFace = "top" | "bottom" | "left" | "right";
export type HvacDuctOutlet = { id: string; type: "hvac-duct-outlet"; ductId: string; segmentId: string; face: HvacOutletFace; offsetMm: number; sizeMm: [number, number]; createdAt: string };
export type HvacThermostat = { id: string; type: "thermostat"; name: string; position: RoutePoint; mount?: DeviceMount; positioning?: DevicePositionReference; sizeMm: [number, number, number]; createdAt: string };
export type HvacControl = { id: string; thermostatId: string; indoorUnitId: string; createdAt: string };
export type HvacWallPenetration = { id: string; type: "hvac-wall-penetration"; wallId: string; segmentId: string; entry: RoutePoint; exit: RoutePoint; openingMm: [number, number]; createdAt: string };
export type HvacOverlay = { visible: boolean; indoorUnits: HvacIndoorUnit[]; ducts: HvacDuct[]; segments: HvacDuctSegment[]; outlets: HvacDuctOutlet[]; thermostats: HvacThermostat[]; controls: HvacControl[]; wallPenetrations: HvacWallPenetration[] };
...
export type DrawingEvidence = { levelId: string; sourceObjectIds: string[]; basis: { kind: 'explicit' | 'derived'; reference: string; assumptions: string[]; confidence: 'high' | 'limited' | 'unknown' } };
...
export type ConduitOverlayDocument = {
  schemaVersion: "2.3";
  /** sha256 fingerprints this revision; projectId identifies the project across revisions. */
  source: { fileName: string; sha256: string; projectId?: string };
  settings: { colors: Record<RoutingSystem, string>; visibleSystems: Record<RoutingSystem, boolean>; sensorVisible: boolean; bendRadiusMm: number; stockLengthMm: number; junctionBoxSizeMm: [number, number, number] };
  ...
  /** Source evidence for generated drawing nodes; calculated values never live here. */
  pointDimensionEvidence: Record<string, DrawingEvidence>;
  constructionAnnotationEvidence: Record<string, DrawingEvidence>;
  pointPositionDimensionLabelPositions: Record<string, number>;
  pointPositionDimensionLineOffsets: Record<string, number>;
  /** Derived 2D dimension IDs deliberately suppressed by the drafter. */
  hiddenPointPositionDimensionIds: string[];
  /** Derived 2D construction annotation IDs deliberately suppressed by the drafter. */
  hiddenConstructionAnnotationIds: string[];
  constructionAnnotationLabelPositions: Record<string, [number, number]>;
  constructionAnnotationLabelPlacementSignatures: Record<string, string>;
  surfaceChases: SurfaceChase[];
  penetrations: Penetration[];
  installationReferencePlanes: InstallationReferencePlane[];
  layoutReferencePlanes: LayoutReferencePlane[];
  hvac: HvacOverlay;
};
...
const deviceSchema = z.object({ id: z.string(), type: z.literal("network-device"), deviceType: z.enum(DEVICE_TYPES), name: z.string(), position: pointSchema, sizeMm: z.tuple([z.number().positive(), z.number().positive(), z.number().positive()]), orientation: vec3Schema, frame: deviceFrameSchema.optional(), mount: deviceMountSchema.optional(), positioning: positioningSchema.optional(), sprinklerDirection: z.enum(SPRINKLER_DIRECTIONS).optional(), systems: z.array(inputSystemSchema), ports: z.array(portSchema), createdAt: z.string() });
...
const layoutReferencePlaneSchema = z.object({ levelId: z.string(), visible: z.boolean(), elevationMm: z.number(), basis: z.enum(["largest-area-ceiling", "derived-default-2700mm", "explicit"]), sourceCeilingId: z.string().optional() });
const hvacSchema = z.object({ visible: z.boolean().optional(), indoorUnits: z.array(z.object({ id: z.string(), type: z.literal("indoor-air-handling-unit"), name: z.string(), position: pointSchema, mount: deviceMountSchema.optional(), sizeMm: z.tuple([z.number().positive(), z.number().positive(), z.number().positive()]), sectionMm: z.tuple([z.number().positive(), z.number().positive()]), rotationYDegrees: z.number(), createdAt: z.string() })).optional(), ducts: z.array(z.object({ id: z.string(), type: z.literal("hvac-duct"), indoorUnitId: z.string(), system: z.enum(["supply", "return"]), segmentIds: z.array(z.string()), createdAt: z.string() })).optional(), segments: z.array(z.object({ id: z.string(), start: pointSchema, end: pointSchema })).optional(), outlets: z.array(z.object({ id: z.string(), type: z.literal("hvac-duct-outlet"), ductId: z.string(), segmentId: z.string(), face: z.enum(["top", "bottom", "left", "right"]), offsetMm: z.number().min(0), sizeMm: z.tuple([z.number().positive(), z.number().positive()]), createdAt: z.string() })).optional(), thermostats: z.array(z.object({ id: z.string(), type: z.literal("thermostat"), name: z.string(), position: pointSchema, mount: deviceMountSchema.optional(), positioning: positioningSchema.optional(), sizeMm: z.tuple([z.number().positive(), z.number().positive(), z.number().positive()]), createdAt: z.string() })).optional(), controls: z.array(z.object({ id: z.string(), thermostatId: z.string(), indoorUnitId: z.string(), createdAt: z.string() })).optional(), wallPenetrations: z.array(z.object({ id: z.string(), type: z.literal("hvac-wall-penetration"), wallId: z.string(), segmentId: z.string(), entry: pointSchema, exit: pointSchema, openingMm: z.tuple([z.number().positive(), z.number().positive()]), createdAt: z.string() })).optional() }).optional();
...
const drawingEvidenceSchema = z.object({ levelId: z.string(), sourceObjectIds: z.array(z.string()), basis: z.object({ kind: z.enum(['explicit', 'derived']), reference: z.string(), assumptions: z.array(z.string()), confidence: z.enum(['high', 'limited', 'unknown']) }) });
...
const overlaySchema = z.object({ schemaVersion: z.enum(["1.0", "1.1", "1.2", "2.0", "2.1", "2.2", "2.3"]), source: z.object({ fileName: z.string(), sha256: z.string(), projectId: z.string().min(1).optional() }), settings: z.object({ colors: z.record(z.string(), z.string()), visibleSystems: z.record(z.string(), z.boolean()).optional(), sensorVisible: z.boolean().optional(), bendRadiusMm: z.number().positive().optional(), stockLengthMm: z.number().positive().optional(), junctionBoxSizeMm: z.tuple([z.number().positive(), z.number().positive(), z.number().positive()]).optional() }), segments: z.array(segmentSchema), fittings: z.array(fittingSchema), junctionBoxes: z.array(junctionBoxSchema).optional(), devices: z.array(deviceSchema).optional(), circuits: z.array(circuitSchema).optional(), lightingControlGroups: z.array(lightingControlGroupSchema).optional(), manualCallouts: z.array(manualCalloutSchema).optional(), pointDimensionEvidence: z.record(z.string(), drawingEvidenceSchema).optional(), constructionAnnotationEvidence: z.record(z.string(), drawingEvidenceSchema).optional(), pointPositionDimensionLabelPositions: z.record(z.string(), z.number().min(0).max(1)).optional(), pointPositionDimensionLineOffsets: z.record(z.string(), z.number()).optional(), hiddenPointPositionDimensionIds: z.array(z.string()).optional(), hiddenConstructionAnnotationIds: z.array(z.string()).optional(), constructionAnnotationLabelPositions: z.record(z.string(), z.tuple([z.number(), z.number()])).optional(), constructionAnnotationLabelPlacementSignatures: z.record(z.string(), z.string()).optional(), wallChases: z.array(wallChaseSchema).optional(), surfaceChases: z.array(surfaceChaseSchema).optional(), penetrations: z.array(penetrationSchema), installationReferencePlanes: z.array(installationReferencePlaneSchema).optional(), layoutReferencePlanes: z.array(layoutReferencePlaneSchema).optional(), hvac: hvacSchema });
...
export function createEmptyOverlay(fileName: string, sha256: string, projectId?: string): ConduitOverlayDocument {
  return { schemaVersion: "2.3", source: { fileName, sha256, ...(projectId ? { projectId } : {}) }, settings: { colors: Object.fromEntries(SYSTEMS.map((system) => [system, SYSTEM_DEFAULTS[system].color])) as Record<RoutingSystem, string>, visibleSystems: Object.fromEntries(SYSTEMS.map((system) => [system, true])) as Record<RoutingSystem, boolean>, sensorVisible: true, bendRadiusMm: DEFAULT_BEND_RADIUS_MM, stockLengthMm: 4000, junctionBoxSizeMm: [86, 86, 50] }, segments: [], fittings: [], junctionBoxes: [], devices: [], circuits: [], lightingControlGroups: [], manualCallouts: [], pointDimensionEvidence: {}, constructionAnnotationEvidence: {}, pointPositionDimensionLabelPositions: {}, pointPositionDimensionLineOffsets: {}, hiddenPointPositionDimensionIds: [], hiddenConstructionAnnotationIds: [], constructionAnnotationLabelPositions: {}, constructionAnnotationLabelPlacementSignatures: {}, surfaceChases: [], penetrations: [], installationReferencePlanes: [], layoutReferencePlanes: [], hvac: { visible: true, indoorUnits: [], ducts: [], segments: [], outlets: [], thermostats: [], controls: [], wallPenetrations: [] } };
}
...
export function parseOverlay(raw: unknown): ConduitOverlayDocument {
  ...
  const devices: NetworkDevice[] = (parsed.devices ?? []).map((device) => {
    const systems = device.deviceType === "sensor" ? [] : device.systems.map((system) => migrateSystem(system as RoutingSystem | LegacyRoutingSystem));
    ...
    const normalizedPorts = device.deviceType === "sensor" ? [] : device.ports.map((value) => normalizePort(value, { kind: "device", id: device.id }, value.system ? migrateSystem(value.system as RoutingSystem | LegacyRoutingSystem) : systems[0] ?? "receptacle", value.segmentId));
  ...
  const hvac = parsed.hvac ? { visible: parsed.hvac.visible ?? true, indoorUnits: parsed.hvac.indoorUnits ?? [], ducts: parsed.hvac.ducts ?? [], segments: parsed.hvac.segments ?? [], outlets: parsed.hvac.outlets ?? [], thermostats: parsed.hvac.thermostats ?? [], controls: parsed.hvac.controls ?? [], wallPenetrations: parsed.hvac.wallPenetrations ?? [] } : { visible: true, indoorUnits: [], ducts: [], segments: [], outlets: [], thermostats: [], controls: [], wallPenetrations: [] };
  return { schemaVersion: "2.3", source: parsed.source, segments, fittings, junctionBoxes, devices, circuits, lightingControlGroups, manualCallouts: parsed.manualCallouts ?? [], pointDimensionEvidence: parsed.pointDimensionEvidence ?? {}, constructionAnnotationEvidence: parsed.constructionAnnotationEvidence ?? {}, pointPositionDimensionLabelPositions: parsed.pointPositionDimensionLabelPositions ?? {}, pointPositionDimensionLineOffsets: parsed.pointPositionDimensionLineOffsets ?? {}, hiddenPointPositionDimensionIds: parsed.hiddenPointPositionDimensionIds ?? [], hiddenConstructionAnnotationIds: parsed.hiddenConstructionAnnotationIds ?? [], constructionAnnotationLabelPositions: parsed.constructionAnnotationLabelPositions ?? {}, constructionAnnotationLabelPlacementSignatures: parsed.constructionAnnotationLabelPlacementSignatures ?? {}, surfaceChases: [...migratedChases, ...(parsed.surfaceChases ?? [])], penetrations, installationReferencePlanes: parsed.installationReferencePlanes ?? [], layoutReferencePlanes: parsed.layoutReferencePlanes ?? [], hvac, settings: { colors, visibleSystems, sensorVisible: parsed.settings.sensorVisible ?? true, bendRadiusMm: DEFAULT_BEND_RADIUS_MM, stockLengthMm: parsed.settings.stockLengthMm ?? 4000, junctionBoxSizeMm: parsed.settings.junctionBoxSizeMm ?? [86, 86, 50] } };
}
