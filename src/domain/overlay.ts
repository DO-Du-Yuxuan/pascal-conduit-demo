...
export type HvacThermostat = { id: string; type: "thermostat"; name: string; position: RoutePoint; mount?: DeviceMount; sizeMm: [number, number, number]; createdAt: string };
...
export function parseOverlay(raw: unknown): ConduitOverlayDocument {
  ...
  const devices: NetworkDevice[] = (parsed.devices ?? []).map((device) => {
    ...
    return { ...device, systems, frame: { front, up, right }, mount: device.mount ?? (device.position.attachment ? { kind: "host", attachment: device.position.attachment } : undefined), ...(device.deviceType === "sprinkler-head" ? { sprinklerDirection: device.sprinklerDirection ?? "upright" } : {}), ports: migrateBoxPorts(device, { front, up, right }, normalizedPorts, parsed.schemaVersion) };
  });
