...
describe("routing collision validation", () => {
  ...
  it("allows contact at a shared physical port", () => {
    const overlay = createEmptyOverlay("a.json", "sha"), existing = planRoute("power", 20, "surface", [point(0, 0, 0), point(1, 0, 0)]);
    existing.segments[0].endPortId = "shared-port";
    const committed = commitPlannedRoute(overlay, existing), extension = planRoute("power", 20, "surface", [point(1, 0, 0), point(2, 0, 0)]);
  ...
  it("checks branch box clearance against other network objects", () => {
    const base = commitPlannedRoute(createEmptyOverlay("a.json", "sha"), planRoute("power", 20, "surface", [point(0, 0, 0), point(2, 0, 0)]));
    const other = commitPlannedRoute(base, planRoute("signal", 20, "surface", [point(1, 0, -.2), point(1, 0, .2)]));
  ...
  it("keeps a 500-segment scene in the lightweight analytic path", () => {
    ...
    for (let index = 0; index < 500; index += 1) overlay = commitPlannedRoute(overlay, planRoute("power", 20, "surface", [point(index * .1, 0, 0), point(index * .1 + .05, 0, 0)]));
