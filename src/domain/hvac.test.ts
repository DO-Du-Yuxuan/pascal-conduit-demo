...
describe('HVAC Overlay', () => {
  ...
  it('starts a supply duct at the indoor unit supply port, never an arbitrary click point', () => {
    ...
    expect(firstSegment?.start.position).toEqual([0, 2.85, 0.3]);
  });
  it('keeps the plan footprint and supply/return ports in the same yaw frame as the 3D unit', () => {
    ...
    expect(indoorUnitPort(unit, 'supply').position).toEqual([1.3, 2.85, 3]);
  ...
  it('uses the selected supply or return port normal as the first duct direction', () => {
    ...
    expect(projectFirstDuctSegmentFromPort(placed.unit, 'supply', point(1, 2.85, 2)).position).toEqual([1, 2.85, 4.6]);
    expect(projectFirstDuctSegmentFromPort(placed.unit, 'supply', point(1, 2.85, 0)).position).toEqual([1, 2.85, 6.6]);
  ...
  it('allows only 90-degree extensions and protects outlets when shortening a terminal', () => {
    ...
    const turned = appendHvacDuctSegment(routed.overlay, routed.duct.id, point(2, 2.85, 2.3));
