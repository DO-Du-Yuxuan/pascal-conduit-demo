...
describe("default Pascal base drawing", () => {
  it("keeps the supplied single-level layout as the empty demo base", () => {
    ...
    expect(counts.wall).toBe(19);
    expect(counts.curvedWall ?? 0).toBe(0);
    expect(Object.values(building.nodes).filter((node) => node.type === "wall" && typeof node.curveOffset === "number" && Math.abs(node.curveOffset) > 1e-9)).toHaveLength(3);
    expect(counts.slab).toBe(3);
    expect(counts.ceiling).toBe(3);
    expect(counts.item).toBe(54);
  });
});
