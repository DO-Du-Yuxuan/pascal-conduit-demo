...
export function planRoute(system: RoutingSystem, diameterMm: number, mode: SurfaceMode, points: RoutePoint[]): PlannedRoute {
  ...
  if (isElectrical(system) && mode === "surface") for (const segment of segments) {
    ...
    if (a?.hostKind === "wall" && a.hostId === b?.hostId) wallChases.push({ id: nextId("chase"), type: "wall-chase", wallId: a.hostId, segmentId: segment.id, start: copyPoint(segment.start), end: copyPoint(segment.end), widthMm: diameterMm + 10, depthMm: diameterMm + 5 });
  }
  ...
  if (mode === "penetrate") for (const segment of segments) for (const point of [segment.start, segment.end]) {
    ...
    if (host) penetrations.push({ id: nextId("penetration"), type: "penetration", hostId: host.hostId, hostKind: host.hostKind, segmentId: segment.id, point: copyPoint(point), diameterMm: diameterMm + 10 });
  }
