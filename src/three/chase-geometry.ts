...
export function subtractHorizontalChases(baseGeometry: BufferGeometry, chases: SurfaceChase[], surfaceY: number, thickness: number): { geometry: BufferGeometry; failed: boolean } {
  ...
  try {
    ...
    let result = new Brush(baseGeometry); result.updateMatrixWorld(true);
    for (const chase of chases) {
      ...
      for (let index = 0; index < points.length - 1; index += 1) {
        ...
        const cutter = new Brush(new BoxGeometry(distance + width, width, depth + .004));
        ...
        result = evaluator.evaluate(result, cutter, SUBTRACTION); result.updateMatrixWorld(true);
      }
    }
