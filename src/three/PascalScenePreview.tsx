...
function Beam({ node, selected, editable = true, onSelect, levelY, preview = false, penetrations = EMPTY_PENETRATIONS, onDrag, onSurfaceHit, onSurfaceMove, onSurfaceFinish }: { node: NodeData; selected: boolean; editable?: boolean; onSelect: () => void; levelY: number; preview?: boolean; penetrations?: Penetration[]; onDrag?: (edit: BeamEdit, commit: boolean) => void; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void; onSurfaceMove?: (hit: ThreeDSurfaceHit | null) => void; onSurfaceFinish?: () => void }) {
  ...
  return <group><mesh data-beam-body={node.id} aria-label="Beam body" raycast={preview ? () => null : undefined} position={[(start[0] + end[0]) / 2, y, (start[2] + end[2]) / 2]} rotation={[0, Math.atan2(dx, dz), 0]} onPointerDown={(event) => { if (!selected || !editable || preview) return; event.stopPropagation(); setBodyDrag({ point: dragPoint(event), start: [start[0], start[2]], end: [end[0], end[2]] }); (event.currentTarget as Element).setPointerCapture(event.pointerId); }} onPointerMove={(event) => { if (preview) return; if (bodyDrag) { const point = dragPoint(event), delta: [number, number] = [point[0] - bodyDrag.point[0], point[1] - bodyDrag.point[1]]; onDrag?.({ start: [bodyDrag.start[0] + delta[0], bodyDrag.start[1] + delta[1]], end: [bodyDrag.end[0] + delta[0], bodyDrag.end[1] + delta[1]] }, false); } else onSurfaceMove?.(surfaceHit(event)); }} onPointerUp={(event) => { if (!bodyDrag) return; const point = dragPoint(event), delta: [number, number] = [point[0] - bodyDrag.point[0], point[1] - bodyDrag.point[1]]; onDrag?.({ start: [bodyDrag.start[0] + delta[0], bodyDrag.start[1] + delta[1]], end: [bodyDrag.end[0] + delta[0], bodyDrag.end[1] + delta[1]] }, true); setBodyDrag(null); }} onClick={(event) => { if (preview) return; event.stopPropagation(); onSelect(); const hit = surfaceHit(event); if (hit && event.nativeEvent.detail < 2) onSurfaceHit?.(hit); }} onDoubleClick={(event) => { if (!preview) onSurfaceFinish?.(); }}><primitive object={beamGeometry} attach="geometry" /><meshStandardMaterial color={selected ? "#fb923c" : "#77808a"} transparent={preview} opacity={preview ? .45 : 1} roughness={.7} /><BoxEdges width={width} height={height} depth={length} opacity={preview ? .5 : undefined} /></mesh>{penetrations.map((penetration) => <mesh key={penetration.id} position={penetration.entry.position}><sphereGeometry args={[penetration.diameterMm / 2000, 12, 8]} /><meshBasicMaterial color="#111827" /></mesh>)}{!preview && handle("start", [start[0], y, start[2]])}{!preview && handle("end", [end[0], y, end[2]])}</group>;
}
...
export function PascalScenePreview({ scene, layers, hiddenNodeIds, levelMode, wallMode, selectedId, highlightHostId, previousRoutePoint, penetrationBypassHostId, beamPreview, beamEditing = true, onBeamDrag, onSelect, overlay, appliedSurfaceChases, appliedPenetrations, constructionMode = "construction", onSurfaceHit, onSurfaceMove, onSurfaceFinish, onChaseFallback }: Props) {
  ...
  return <group name="pascal-readonly-preview">
    ...
    {sceneIndex.nodes.map((node) => {
      ...
      if (node.type === "beam") { const rendered = beamPreview?.id === node.id ? beamPreview : node; return <Beam key={node.id} node={rendered} selected={selected} editable={beamEditing} onSelect={select} levelY={elevation(rendered)} penetrations={constructionMode === "finished" ? EMPTY_PENETRATIONS : penetrationsByHost[node.id] ?? EMPTY_PENETRATIONS} onDrag={(edit, commit) => onBeamDrag?.(node.id, edit, commit)} onSurfaceHit={handleSurfaceHit} onSurfaceMove={handleSurfaceMove} onSurfaceFinish={onSurfaceFinish} />; }
