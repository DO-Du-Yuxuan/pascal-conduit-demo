...
import type { ConduitOverlayDocument, HostAttachment, Penetration, RoutePoint, SurfaceChase, Vec3 } from "../domain/overlay";
...
import { beamClearances, beamPlanarClearances, type BeamClearance, type BeamPlanarClearance } from "../domain/beam-positioning";
...
function BeamDimensionGuides({ beam, clearances, y }: { beam: BeamNode; clearances: BeamClearance[]; y: number }) {
  ...
  return <group name="beam-position-dimensions">{clearances.map((clearance) => {
    const direction = clearance.edge === "left" ? normal : clearance.edge === "right" ? [-normal[0], -normal[1]] as [number, number] : clearance.edge === "start" ? [-axis[0], -axis[1]] as [number, number] : axis;
    const origin2 = clearance.edge === "left" ? [center[0] + normal[0] * beam.width / 2, center[1] + normal[1] * beam.width / 2] : clearance.edge === "right" ? [center[0] - normal[0] * beam.width / 2, center[1] - normal[1] * beam.width / 2] : clearance.edge === "start" ? beam.start : beam.end;
    const start: Point = [origin2[0], y, origin2[1]], end: Point = [origin2[0] + direction[0] * clearance.meters, y, origin2[1] + direction[1] * clearance.meters], middle: Point = [(start[0] + end[0]) / 2, y, (start[2] + end[2]) / 2];
    return <group key={clearance.edge} data-beam-clearance-dimension={clearance.edge}><Line points={[start, end]} color="#f97316" lineWidth={1.5} raycast={() => null} /><mesh position={start} raycast={() => null}><sphereGeometry args={[.02, 8, 6]} /><meshBasicMaterial color="#f97316" /></mesh><mesh position={end} raycast={() => null}><sphereGeometry args={[.02, 8, 6]} /><meshBasicMaterial color="#f97316" /></mesh><Html position={middle} center distanceFactor={9}><span className="conduit-dimension-label">{Math.round(clearance.meters * 1000)} mm</span></Html></group>;
  })}</group>;
}
...
function Beam({ node, selected, onSelect, levelY, preview = false, penetrations = EMPTY_PENETRATIONS, clearances = [], planarClearances = [], onSurfaceHit, onSurfaceMove, onSurfaceFinish }: { node: NodeData; selected: boolean; onSelect: () => void; levelY: number; preview?: boolean; penetrations?: Penetration[]; clearances?: BeamClearance[]; planarClearances?: BeamPlanarClearance[]; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void; onSurfaceMove?: (hit: ThreeDSurfaceHit | null) => void; onSurfaceFinish?: () => void }) {
  ...
  const surfaceHit = (event: any): ThreeDSurfaceHit | null => {
    const p: Vec3 = [event.point.x, event.point.y, event.point.z], axis: Vec3 = [dx / length, 0, dz / length], side: Vec3 = [-axis[2], 0, axis[0]], center: Vec3 = [(start[0] + end[0]) / 2, 0, (start[2] + end[2]) / 2];
    const delta: Vec3 = [p[0] - center[0], 0, p[2] - center[2]], along = delta[0] * axis[0] + delta[2] * axis[2], across = delta[0] * side[0] + delta[2] * side[2], bottom = levelY + top - height;
    const candidates: Array<{ surface: string; distance: number; normal: Vec3; u: Vec3; v: Vec3 }> = [
      { surface: "bottom", distance: Math.abs(p[1] - bottom), normal: [0, -1, 0], u: axis, v: side },
      { surface: "side-a", distance: Math.abs(across + width / 2), normal: [-side[0], 0, -side[2]], u: axis, v: [0, 1, 0] },
      { surface: "side-b", distance: Math.abs(across - width / 2), normal: side, u: axis, v: [0, 1, 0] },
      { surface: "end-a", distance: Math.abs(along + length / 2), normal: [-axis[0], 0, -axis[2]], u: side, v: [0, 1, 0] },
      { surface: "end-b", distance: Math.abs(along - length / 2), normal: axis, u: side, v: [0, 1, 0] },
    ];
  ...
  const beamGeometry = useMemo(() => subtractBeamPenetrations(new BoxGeometry(width, height, length), penetrations, [start[0], 0, start[2]], [end[0], 0, end[2]], y).geometry, [width, height, length, penetrations, start[0], start[2], end[0], end[2], y]);
  useEffect(() => () => beamGeometry.dispose(), [beamGeometry]);
  return <group><mesh data-beam-body={node.id} aria-label="Beam body" raycast={preview ? () => null : undefined} position={[(start[0] + end[0]) / 2, y, (start[2] + end[2]) / 2]} rotation={[0, Math.atan2(dx, dz), 0]} onPointerMove={(event) => { if (!preview) onSurfaceMove?.(surfaceHit(event)); }} onClick={(event) => { if (preview) return; event.stopPropagation(); onSelect(); const hit = surfaceHit(event); if (hit && event.nativeEvent.detail < 2) onSurfaceHit?.(hit); }} onDoubleClick={(event) => { if (!preview) onSurfaceFinish?.(); }}><primitive object={beamGeometry} attach="geometry" /><meshStandardMaterial color={selected ? "#fb923c" : "#77808a"} transparent={preview} opacity={preview ? .45 : 1} roughness={.7} /><BoxEdges width={width} height={height} depth={length} opacity={preview ? .5 : undefined} /></mesh>{penetrations.map((penetration) => <mesh key={penetration.id} position={penetration.entry.position}><sphereGeometry args={[penetration.diameterMm / 2000, 12, 8]} /><meshBasicMaterial color="#111827" /></mesh>)}{selected && !preview && <><BeamPlanarDimensionGuides beam={node as BeamNode} clearances={planarClearances} y={levelY + top - height - .05} /><BeamDimensionGuides beam={node as BeamNode} clearances={clearances} y={levelY + top - height - .035} /></>}</group>;
}
...
function Surface({ node, y, thickness = 0, color, opacity, selected, onSelect, attachment, chases = EMPTY_CHASES, penetrations = EMPTY_PENETRATIONS, onSurfaceHit, onSurfaceMove, onSurfaceFinish, onChaseFallback }: { node: NodeData; y: number; thickness?: number; color: string; opacity: number; selected: boolean; onSelect: () => void; attachment?: HostAttachment; chases?: SurfaceChase[]; penetrations?: Penetration[]; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void; onSurfaceMove?: (hit: ThreeDSurfaceHit | null) => void; onSurfaceFinish?: () => void; onChaseFallback?: (key: string, failed: boolean) => void }) {
  const result = useMemo(() => {
    ...
    if (polygon.length < 3) return { geometry: null, failed: false };
    ...
    for (const penetration of penetrations) {
      ...
      for (let index = 0; index < 12; index += 1) { const angle = index / 12 * Math.PI * 2, x = penetration.entry.position[0] + Math.cos(angle) * radius, z = penetration.entry.position[2] + Math.sin(angle) * radius; if (index) path.lineTo(x, -z); else path.moveTo(x, -z); }
      path.closePath(); shape.holes.push(path);
    }
    if (thickness <= 0) return { geometry: new ShapeGeometry(shape), failed: false };
    const base = new ExtrudeGeometry(shape, { depth: thickness, bevelEnabled: false });
    return subtractHorizontalChases(base, chases, y, thickness);
  }, [node.polygon, node.holes, penetrations, chases, thickness, y]);
  useEffect(() => { onChaseFallback?.(node.id, result.failed); return () => onChaseFallback?.(node.id, false); }, [node.id, result.failed, result.geometry, onChaseFallback]);
...
type WallPartSpec = { x: number; y: number; width: number; height: number };
function WallPartMesh({ diagnosticKey, part, start, end, wallY, thickness, chases, selected, translucent, hit, onSelect, onSurfaceHit, onSurfaceMove, onSurfaceFinish, onChaseFallback }: { diagnosticKey: string; part: WallPartSpec; start: Vec3; end: Vec3; wallY: number; thickness: number; chases: SurfaceChase[]; selected: boolean; translucent: boolean; hit: (event: any) => ThreeDSurfaceHit; onSelect: () => void; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void; onSurfaceMove?: (hit: ThreeDSurfaceHit | null) => void; onSurfaceFinish?: () => void; onChaseFallback?: (key: string, failed: boolean) => void }) {
  const result = useMemo(() => subtractWallChases(new BoxGeometry(part.width, part.height, thickness), chases, start, end, wallY, part.x, part.y, thickness), [part.width, part.height, part.x, part.y, thickness, chases, start[0], start[2], end[0], end[2], wallY]);
...
function Wall({ node, hostId, levelId, openings, chases = EMPTY_CHASES, penetrations = EMPTY_PENETRATIONS, y, selected, wallMode, onSelect, onSurfaceHit, onSurfaceMove, onSurfaceFinish, onChaseFallback, curveT }: { node: NodeData; hostId: string; levelId: string | null; openings: NodeData[]; chases?: SurfaceChase[]; penetrations?: Penetration[]; y: number; selected: boolean; wallMode: ThreeDWallMode; onSelect: () => void; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void; onSurfaceMove?: (hit: ThreeDSurfaceHit | null) => void; onSurfaceFinish?: () => void; onChaseFallback?: (key: string, failed: boolean) => void; curveT?: number }) {
  ...
  for (const penetration of penetrations) {
    const center = project(penetration.entry.position), radius = penetration.diameterMm / 2000;
...
function CurvedWall({ node, hostId, levelId, openings, chases = EMPTY_CHASES, penetrations = EMPTY_PENETRATIONS, y, selected, wallMode, onSelect, onSurfaceHit, onSurfaceMove, onSurfaceFinish, onChaseFallback }: { node: NodeData; hostId: string; levelId: string | null; openings: NodeData[]; chases?: SurfaceChase[]; penetrations?: Penetration[]; y: number; selected: boolean; wallMode: ThreeDWallMode; onSelect: () => void; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void; onSurfaceMove?: (hit: ThreeDSurfaceHit | null) => void; onSurfaceFinish?: () => void; onChaseFallback?: (key: string, failed: boolean) => void }) {
...
export function PascalScenePreview({ scene, layers, hiddenNodeIds, levelMode, wallMode, selectedId, highlightHostId, previousRoutePoint, penetrationBypassHostId, beamPreview, onSelect, overlay, appliedSurfaceChases, appliedPenetrations, constructionMode = "construction", onSurfaceHit, onSurfaceMove, onSurfaceFinish, onChaseFallback }: Props) {
  const sceneIndex = useMemo(() => {
    ...
    const levelIdFor = (node: NodeData) => {
      if (node.id in levelIdByNode) return levelIdByNode[node.id];
      let cursor: NodeData | undefined = node, visited = new Set<string>(), result: string | null = null;
      while (cursor && !visited.has(cursor.id)) { visited.add(cursor.id); if (cursor.type === "level") { result = cursor.id; break; } cursor = cursor.parentId ? scene.nodes[cursor.parentId] : undefined; }
      levelIdByNode[node.id] = result;
      return result;
    };
    for (const node of nodes) {
      const levelId = levelIdFor(node), levelNode = levelId ? scene.nodes[levelId] : undefined;
      levelValueByNode[node.id] = levelNode ? numeric(levelNode.level) : 0;
      if ((node.type === "door" || node.type === "window") && (node.wallId || node.parentId)) (openingsByWall[node.wallId ?? node.parentId!] ??= []).push(node);
      if (node.type === "slab") (slabsByLevel[levelId ?? ""] ??= []).push(node);
    }
    return { nodes, levelIdByNode, levelValueByNode, openingsByWall, slabsByLevel };
  }, [scene.nodes]);
  const levelFor = (node: NodeData) => sceneIndex.levelValueByNode[node.id] ?? (node.parentId ? numeric(scene.nodes[node.parentId]?.level) : 0);
  const levelIdFor = (node: NodeData) => sceneIndex.levelIdByNode[node.id] ?? null;
  const wallHosts = useMemo<WallHostCandidate[]>(() => sceneIndex.nodes.filter((node) => node.type === "wall").map((node) => ({
    node,
  ...
  return <group name="pascal-readonly-preview">
    ...
    {sceneIndex.nodes.map((node) => {
      ...
      if (node.type === "beam") { const rendered = beamPreview?.id === node.id ? beamPreview : node, valid = validateBeam(rendered, scene.nodes).beam; return <Beam key={node.id} node={rendered} selected={selected} onSelect={select} levelY={elevation(rendered)} penetrations={constructionMode === "finished" ? EMPTY_PENETRATIONS : penetrationsByHost[node.id] ?? EMPTY_PENETRATIONS} clearances={selected && valid ? beamClearances(scene.nodes, valid) : []} planarClearances={selected && valid ? beamPlanarClearances(scene.nodes, valid) : []} onSurfaceHit={handleSurfaceHit} onSurfaceMove={handleSurfaceMove} onSurfaceFinish={onSurfaceFinish} />; }
      ...
      if (node.type === "item" || node.type === "shelf" || node.type === "cabinet" || node.type === "cabinet-module") {
        ...
        const itemLevelId = levelIdFor(node), grounded = node.asset?.attachTo === "wall-side" || node.asset?.attachTo === "ceiling" ? 0 : floorTopAt(itemLevelId, transform.status === "ok" ? transform.x : position[0], transform.status === "ok" ? transform.z : position[2]);
