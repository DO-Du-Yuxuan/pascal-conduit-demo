...
export type ThreeDSurfaceHit = { point: Vec3; attachment: HostAttachment };
type Props = { scene: ThreeDSceneInput; layers: ThreeDLayerVisibility; hiddenNodeIds: ReadonlySet<string>; levelMode: ThreeDLevelMode; wallMode: ThreeDWallMode; selectedId: string | null; onSelect: (id: string) => void; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void };
...
function Surface({ node, y, thickness = 0, color, opacity, selected, onSelect, attachment, onSurfaceHit }: { node: NodeData; y: number; thickness?: number; color: string; opacity: number; selected: boolean; onSelect: () => void; attachment?: HostAttachment; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void }) {
  ...
  return <mesh position={[0, y, 0]} rotation={[-Math.PI / 2, 0, 0]} onClick={(event) => { event.stopPropagation(); onSelect(); if (attachment) onSurfaceHit?.({ point: [event.point.x, event.point.y, event.point.z], attachment }); }}>
...
function Wall({ node, hostId, levelId, openings, y, selected, wallMode, onSelect, onSurfaceHit }: { node: NodeData; hostId: string; levelId: string | null; openings: NodeData[]; y: number; selected: boolean; wallMode: ThreeDWallMode; onSelect: () => void; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void }) {
  ...
  if (isCurvedWall(node as any)) return <CurvedWall node={node} hostId={hostId} levelId={levelId} openings={openings} y={y} selected={selected} wallMode={wallMode} onSelect={onSelect} onSurfaceHit={onSurfaceHit} />;
...
function CurvedWall({ node, hostId, levelId, openings, y, selected, wallMode, onSelect, onSurfaceHit }: { node: NodeData; hostId: string; levelId: string | null; openings: NodeData[]; y: number; selected: boolean; wallMode: ThreeDWallMode; onSelect: () => void; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void }) {
  ...
  return <group>{Array.from({ length: segments }, (_, index) => {
    ...
    return <Wall key={index} node={{ ...node, id: `${node.id}:curve:${index}`, start: [a.x, a.y], end: [b.x, b.y], curveOffset: 0 }} hostId={hostId} levelId={levelId} openings={segmentOpenings} y={y} selected={selected} wallMode={wallMode} onSelect={onSelect} onSurfaceHit={onSurfaceHit} />;
  })}</group>;
}
...
export function PascalScenePreview({ scene, layers, hiddenNodeIds, levelMode, wallMode, selectedId, onSelect, onSurfaceHit }: Props) {
  ...
  return <group name="pascal-readonly-preview">
    ...
    {Object.values(scene.nodes).map((node) => {
      ...
      if (node.type === "slab") return <Surface key={node.id} node={node} y={y} thickness={Math.max(.01, Math.abs(numeric(node.elevation, .05)))} color="#c4a484" opacity={1} selected={selected} onSelect={select} attachment={{ hostId: node.id, hostKind: "slab", surface: "top", normal: [0, 1, 0], levelId: levelIdFor(node) }} onSurfaceHit={onSurfaceHit} />;
      if (node.type === "ceiling") return <Surface key={node.id} node={node} y={y + numeric(node.height, 2.7)} thickness={.04} color="#f7f2e8" opacity={.94} selected={selected} onSelect={select} attachment={{ hostId: node.id, hostKind: "ceiling", surface: "ceiling-face", normal: [0, -1, 0], levelId: levelIdFor(node) }} onSurfaceHit={onSurfaceHit} />;
