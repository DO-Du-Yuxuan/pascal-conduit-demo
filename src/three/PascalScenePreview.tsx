...
function Surface({ node, y, thickness = 0, color, opacity, selected, onSelect, attachment, penetrations = [], onSurfaceHit, onSurfaceMove, onSurfacePointerDown, onSurfaceFinish }: { node: NodeData; y: number; thickness?: number; color: string; opacity: number; selected: boolean; onSelect: () => void; attachment?: HostAttachment; penetrations?: Penetration[]; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void; onSurfaceMove?: (hit: ThreeDSurfaceHit) => void; onSurfacePointerDown?: (hit: ThreeDSurfaceHit, button: number) => void; onSurfaceFinish?: () => void }) {
  ...
  return <mesh position={[0, y, 0]} rotation={[-Math.PI / 2, 0, 0]} onPointerMove={(event) => { const next = hit(event); if (next) onSurfaceMove?.(next); }} onPointerDown={(event) => { const next = hit(event); if (next) onSurfacePointerDown?.(next, event.button); }} onClick={(event) => { event.stopPropagation(); onSelect(); const next = hit(event); if (next && event.nativeEvent.detail < 2) onSurfaceHit?.(next); }} onDoubleClick={(event) => { event.stopPropagation(); onSurfaceFinish?.(); }}>
...
function Wall({ node, hostId, levelId, openings, chases = [], penetrations = [], y, selected, wallMode, onSelect, onSurfaceHit, onSurfaceMove, onSurfacePointerDown, onSurfaceFinish, curveT }: { node: NodeData; hostId: string; levelId: string | null; openings: NodeData[]; chases?: WallChase[]; penetrations?: Penetration[]; y: number; selected: boolean; wallMode: ThreeDWallMode; onSelect: () => void; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void; onSurfaceMove?: (hit: ThreeDSurfaceHit) => void; onSurfacePointerDown?: (hit: ThreeDSurfaceHit, button: number) => void; onSurfaceFinish?: () => void; curveT?: number }) {
  ...
  return <group position={[(start[0] + end[0]) / 2, y, (start[2] + end[2]) / 2]} rotation={[0, -Math.atan2(end[2] - start[2], end[0] - start[0]), 0]} onPointerMove={(event) => onSurfaceMove?.(hit(event))} onPointerDown={(event) => onSurfacePointerDown?.(hit(event), event.button)} onClick={(event) => { event.stopPropagation(); onSelect(); if (event.nativeEvent.detail < 2) onSurfaceHit?.(hit(event)); }} onDoubleClick={(event) => { event.stopPropagation(); onSurfaceFinish?.(); }}>
...
export function PascalScenePreview({ scene, layers, hiddenNodeIds, levelMode, wallMode, selectedId, highlightHostId, onSelect, overlay, constructionMode = "construction", onSurfaceHit, onSurfaceMove, onSurfacePointerDown, onSurfaceFinish }: Props) {
