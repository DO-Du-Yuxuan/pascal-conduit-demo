...
function BeamPlanarDimensionGuides({ beam, clearances, y }: { beam: BeamNode; clearances: BeamPlanarClearance[]; y: number }) {
  return <group name="beam-planar-position-dimensions">{clearances.map((clearance) => {
    ...
    return <group key={`${clearance.witness.id}:${clearance.witness.face}`} name="beam-planar-dimension:cross-axis"><Line points={[start, end]} color="#f97316" lineWidth={1.5} raycast={() => null} /><mesh position={start} raycast={() => null}><sphereGeometry args={[.02, 8, 6]} /><meshBasicMaterial color="#f97316" /></mesh><mesh position={end} raycast={() => null}><sphereGeometry args={[.02, 8, 6]} /><meshBasicMaterial color="#f97316" /></mesh><Html position={middle} center distanceFactor={9}><span className="conduit-dimension-label">{Math.round(clearance.meters * 1000)} mm</span></Html></group>;
  })}</group>;
}
