...
function BeamPlanarDimensionGuides({ beam, clearances, y }: { beam: BeamNode; clearances: BeamPlanarClearance[]; y: number }) {
  return <group name="beam-planar-position-dimensions">{clearances.map((clearance) => {
    ...
    return <group key={`${clearance.witness.id}:${clearance.witness.face}`} data-beam-planar-dimension="cross-axis"><Line points={[start, end]} color="#f97316" lineWidth={1.5} raycast={() => null} /><mesh position={start} raycast={() => null}><sphereGeometry args={[.02, 8, 6]} /><meshBasicMaterial color="#f97316" /></mesh><mesh position={end} raycast={() => null}><sphereGeometry args={[.02, 8, 6]} /><meshBasicMaterial color="#f97316" /></mesh><Html position={middle} center distanceFactor={9}><span className="conduit-dimension-label">{Math.round(clearance.meters * 1000)} mm</span></Html></group>;
  })}</group>;
}

function Beam({ node, selected, onSelect, levelY, preview = false, penetrations = EMPTY_PENETRATIONS, planarClearances = [], onSurfaceHit, onSurfaceMove, onSurfaceFinish }: { node: NodeData; selected: boolean; onSelect: () => void; levelY: number; preview?: boolean; penetrations?: Penetration[]; planarClearances?: BeamPlanarClearance[]; onSurfaceHit?: (hit: ThreeDSurfaceHit) => void; onSurfaceMove?: (hit: ThreeDSurfaceHit | null) => void; onSurfaceFinish?: () => void }) {
...
export function PascalScenePreview({ scene, layers, hiddenNodeIds, levelMode, wallMode, selectedId, highlightHostId, previousRoutePoint, penetrationBypassHostId, beamPreview, onSelect, overlay, appliedSurfaceChases, appliedPenetrations, constructionMode = "construction", onSurfaceHit, onSurfaceMove, onSurfaceFinish, onChaseFallback }: Props) {
  ...
  return <group name="pascal-readonly-preview">
    ...
    {sceneIndex.nodes.map((node) => {
      ...
      if (node.type === "beam") { const rendered = beamPreview?.id === node.id ? beamPreview : node, valid = validateBeam(rendered, scene.nodes).beam; return <Beam key={node.id} node={rendered} selected={selected} onSelect={select} levelY={elevation(rendered)} penetrations={constructionMode === "finished" ? EMPTY_PENETRATIONS : penetrationsByHost[node.id] ?? EMPTY_PENETRATIONS} planarClearances={selected && valid ? beamPlanarClearances(scene.nodes, valid) : []} onSurfaceHit={handleSurfaceHit} onSurfaceMove={handleSurfaceMove} onSurfaceFinish={onSurfaceFinish} />; }
