...
import { projectRayToActiveWall, routeCursorFromActiveWall } from "./active-host";
...
describe("active wall pointer projection", () => {
  ...
  it("lets an explicitly hit Beam face become the next route host", () => {
    const beamHit: RoutePoint = { position: [2.5, 2.5, .3], attachment: { hostId: "beam-a", hostKind: "beam", surface: "bottom", normal: [0, -1, 0], levelId: "L0", localPosition: [0, 2.5, 0], basis: { u: [1, 0, 0], v: [0, 0, 1] } } };
    expect(routeCursorFromActiveWall(wall, active, [2.5, 2, 5], [0, 0, -1], beamHit)).toBe(beamHit);
  });
});
