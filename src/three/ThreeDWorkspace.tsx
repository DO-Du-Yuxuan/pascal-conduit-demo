...
import { constrainToHostAxes, previewRoutePoints } from "../domain/drawing";
...
export default function ThreeDWorkspace({ scene, hiddenNodeIds, selectedId, onSelect, sourceFile, sourceSha }: { scene: ThreeDSceneInput | null; hiddenNodeIds: ReadonlySet<string>; selectedId: string | null; onSelect: (id: string | null) => void; sourceFile: string; sourceSha: string }) {
  ...
  const [tool, setTool] = useState<Tool>("select"), [system, setSystem] = useState<RoutingSystem>("power"), [diameterMm, setDiameterMm] = useState(20), [surfaceMode, setSurfaceMode] = useState<SurfaceMode>("surface"), [constructionParameters, setConstructionParameters] = useState<ConstructionVisualParameters>({ chaseWidthMm: 30, chaseDepthMm: 25, penetrationDiameterMm: 30 }), [draft, setDraft] = useState<RoutePoint[]>([]), [cursor, setCursor] = useState<RoutePoint | null>(null), [cursorShiftKey, setCursorShiftKey] = useState(false), [panelCollapsed, setPanelCollapsed] = useState(false), [pending, setPending] = useState<PlannedRoute | null>(null), [branchStart, setBranchStart] = useState<BranchStart | null>(null), [penetrationEntry, setPenetrationEntry] = useState<RoutePoint | null>(null), [constructionMode, setConstructionMode] = useState<ConstructionMode>("construction"), [status, setStatus] = useState("Overlay 为空；选择系统后可在 3D 中开始画管。");
  ...
  const previewPoints = useMemo(() => previewRoutePoints(draft, cursor, "free", cursorShiftKey), [draft, cursor, cursorShiftKey]);
  ...
  const onSurfaceMove = (hit: ThreeDSurfaceHit) => {
    if (tool === "draw" || (tool === "branch" && branchStart) || (tool === "penetrate" && penetrationEntry)) { setCursor(routePoint(hit)); setCursorShiftKey(hit.shiftKey); }
  };
  const onSurfaceHit = (hit: ThreeDSurfaceHit) => {
    ...
    if (tool !== "draw") return;
    setDraft((points) => [...points, points.length ? constrainToHostAxes(points[points.length - 1], point, hit.shiftKey ? "orthogonal" : "free") : point]); setCursor(null); setStatus("已确定落点；移动鼠标预览下一段，按住 Shift 可临时正交，Enter 或“完成路径”进入施工影响确认。");
  };
