...
const thermostatPositioningProxy = (thermostat: HvacThermostat): NetworkDevice => {
  ...
  return { id: thermostat.id, type: "network-device", deviceType: "switch", name: thermostat.name, position: thermostat.position, mount: thermostat.mount, positioning: thermostat.positioning, sizeMm: thermostat.sizeMm, orientation: frame.front, frame, systems: ["lighting"], ports: [], createdAt: thermostat.createdAt };
};
...
function InstallationPlane({ bounds, y, onPreview, onPlace }: { bounds: ThreeDBounds; y: number; onPreview: (position: [number, number, number] | null) => void; onPlace: (position: [number, number, number]) => void }) {
  // Device placement intentionally shares the exact Layout-plane interaction
  // geometry; only its persisted mount relationship is different.
...
export default function ThreeDWorkspace({ scene, hiddenNodeIds, selectedId, onSelect, sourceFile, sourceSha, projectId }: { scene: ThreeDSceneInput | null; hiddenNodeIds: ReadonlySet<string>; selectedId: string | null; onSelect: (id: string | null) => void; sourceFile: string; sourceSha: string; projectId: string | null }) {
  ...
  const applyHvacPosition = (change: { bottomElevationMm?: number; planarClearanceMm?: Partial<Record<'px' | 'nx' | 'pz' | 'nz', number>> }) => {
    ...
    for (const reference of selectedHvacPlanarReferences) {
      ...
      const delta = (reference.millimeters - requested) / 1000;
      if (reference.axis === 'x') nextPosition[0] += reference.sign * delta;
      else nextPosition[2] += reference.sign * delta;
    }
  ...
  const finishHvacDuct = () => {
    if ((tool !== 'hvac-supply' && tool !== 'hvac-return') || (!hvacRouteStart && !activeHvacDuctId)) return;
    setActiveHvacDuctId(null); setHvacRouteStart(null); setTool('select'); setStatus('风管路径已完成。');
  };
  ...
  const onSurfaceHit = (hit: ThreeDSurfaceHit) => {
    ...
    if (tool === "hvac-supply" || tool === "hvac-return") {
      ...
      if (!activeHvacDuctId) {
        ...
        if ('reason' in result) { setStatus(result.reason); return; }
        commit(result.overlay); setActiveHvacDuctId(result.duct.id); setStatus('已创建第一段风管；每次单击确认一个点，双击或 Enter 完成。'); return;
      }
  ...
  const beamPointerLabel = beamPointerState === "surface-snap" ? `Surface snap · ${beamSnapTarget!.target.kind} · ${beamSnapTarget!.target.id}`
    : beamPointerState === "crossing" ? `Explicit Ceiling elevation crossing · 保留 ${Math.round((activeBeamCandidate!.beam!.effectiveCeilingElevation.meters) * 1000)} mm`
      : beamPointerState === "invalid" ? activeBeamCandidate?.diagnostics[0] ?? "当前 Beam 无效。"
        : beamPointerState === "free" ? "自由有效点" : null;
  const onPointerRay = (origin: [number, number, number], direction: [number, number, number], shiftKey = false, ctrlKey = false) => {
