...
export default function ThreeDWorkspace({ scene, hiddenNodeIds, selectedId, onSelect, sourceFile, sourceSha }: { scene: ThreeDSceneInput | null; hiddenNodeIds: ReadonlySet<string>; selectedId: string | null; onSelect: (id: string | null) => void; sourceFile: string; sourceSha: string }) {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Enter" && draft.length > 1) { event.preventDefault(); setPending(planRoute(system, diameterMm, surfaceMode, draft)); setDraft([]); setCursor(null); }
  ...
  const finishPath = () => { if (draft.length < 2) return; setPending(planRoute(system, diameterMm, surfaceMode, draft)); setDraft([]); setCursor(null); };
  ...
  const onSurfaceHit = (hit: ThreeDSurfaceHit) => {
    ...
    if (tool === "penetrate") {
      ...
      setPending(planRoute(system, diameterMm, "penetrate", [penetrationEntry, point])); setPenetrationEntry(null); setCursor(null); return;
    }
    if (tool !== "draw") return;
