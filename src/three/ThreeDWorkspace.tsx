...
function Navigation({ bounds, preset, pivot }: { bounds: ThreeDBounds; preset: ViewPreset; pivot: [number, number, number] | null }) {
  ...
  useEffect(() => {
    if (!pivot || !controls.current) return;
    const controlsTarget = controls.current.target, deltaX = pivot[0] - controlsTarget.x, deltaY = pivot[1] - controlsTarget.y, deltaZ = pivot[2] - controlsTarget.z;
    // Move camera and target together first: changing the orbit pivot must not
    // alter the frame until the user actually begins to drag.
    controls.current.object.position.add({ x: deltaX, y: deltaY, z: deltaZ });
    controlsTarget.set(...pivot); controls.current.update();
  }, [pivot]);
