...
function Navigation({ bounds, preset, pivot }: { bounds: ThreeDBounds; preset: ViewPreset; pivot: [number, number, number] | null }) {
  ...
  useEffect(() => { if (pivot && controls.current) { controls.current.target.set(...pivot); controls.current.update(); } }, [pivot]);
...
function PointerCapture({ bounds, onRay, onPivot }: { bounds: ThreeDBounds; onRay: (origin: [number, number, number], direction: [number, number, number]) => void; onPivot: (point: [number, number, number]) => void }) {
  const radius = Math.max(10, bounds.span * 4);
  return <mesh position={bounds.center} onPointerMove={(event: ThreeEvent<PointerEvent>) => onRay([event.ray.origin.x, event.ray.origin.y, event.ray.origin.z], [event.ray.direction.x, event.ray.direction.y, event.ray.direction.z])} onPointerDown={(event: ThreeEvent<PointerEvent>) => { if (event.button === 2) onPivot([event.point.x, event.point.y, event.point.z]); }}>
