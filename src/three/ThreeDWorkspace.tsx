...
function LayoutPlane({ bounds, y, onPreview, onPlace }: { bounds: ThreeDBounds; y: number; onPreview: (position: [number, number, number] | null) => void; onPlace: (position: [number, number, number]) => void }) {
  const size = Math.max(4, bounds.span * 1.6);
  return <mesh name="layout-reference-plane" position={[bounds.center[0], y, bounds.center[2]]} rotation={[-Math.PI / 2, 0, 0]} onPointerMove={(event) => { event.stopPropagation(); onPreview([event.point.x, y, event.point.z]); }} onPointerOut={() => onPreview(null)} onClick={(event) => { if (event.nativeEvent.button !== 0) return; event.stopPropagation(); onPlace([event.point.x, y, event.point.z]); }}>
...
export default function ThreeDWorkspace({ scene, hiddenNodeIds, selectedId, onSelect, sourceFile, sourceSha, projectId }: { scene: ThreeDSceneInput | null; hiddenNodeIds: ReadonlySet<string>; selectedId: string | null; onSelect: (id: string | null) => void; sourceFile: string; sourceSha: string; projectId: string | null }) {
  ...
  const beamSnapTarget = scene && beamPointer && (beamStart?.attachment.levelId ?? beamPointer.levelId) ? snapBeamPoint(scene.nodes, beamStart?.attachment.levelId ?? beamPointer.levelId!, [beamPointer.point[0], beamPointer.point[2]]) : null;
  ...
  const onLayoutReferenceHit = (point: [number, number, number]) => {
    ...
    onSurfaceHit({ point, shiftKey: false, attachment: { hostId: `layout-reference-plane:${activeLevelId}`, hostKind: "ceiling", surface: "layout-reference-plane", normal: [0, -1, 0], levelId: activeLevelId } });
  };
  ...
  <Canvas key={projection} frameloop="demand" orthographic={projection === "orthographic"} camera={projection === "orthographic" ? ORTHOGRAPHIC_CAMERA : PERSPECTIVE_CAMERA} dpr={CANVAS_DPR} gl={CANVAS_GL} onPointerMissed={() => selectWhileBrowsing(null)}><color attach="background" args={["#dfe6e9"]} />
    {tool === "beam" && layoutReferencePlane?.visible && <LayoutPlane bounds={scene.bounds} y={layoutReferencePlaneY} onPreview={(position) => setBeamPointer(position ? { point: position, shiftKey: false, levelId: activeLevelId } : null)} onPlace={onLayoutReferenceHit} />}
