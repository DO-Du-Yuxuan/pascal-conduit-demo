...
The exposed **bottom**, two **sides**, and two **ends** of a valid Beam are ordinary conduit hosts. Every attachment records the Beam id, named face, outward normal, local basis, Level, and local position; the Ceiling-adjacent top is intentionally not pickable. When a route leaves a Wall, an explicit hit on one of these Beam faces takes priority over the Wall's continuous cursor projection, so its preview and next confirmed point truthfully land on the Beam; Ceiling-to-Beam routing already uses the direct host hit. Existing guided drawing, bends, turns, and completion therefore work across adjacent Beam faces and between Beam and other hosts without changing the source building model.
